...
/*
 * Implementing class for the EmployeeDao interface
 */
...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	/*
	 * Implementing registerEmployee Method Register new Employee into system and
	 * save it to database.
	 */
	@Override
	public String registerEmployee(EmployeeDtoReg dtoReg) throws EmployeeException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO EMPLOYEE (name, email, password, status) VALUES(?,?,?,?)");
	...
	/*
	 * Implementing loginEmployee method Validating new employee Checking if HOD
	 * approves the employee or not. Getting details of employee.
	 */
	...
	public int loginEmployee(EmployeeDtoCred cred) throws EmployeeException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			PreparedStatement ps = conn
					.prepareStatement("SELECT empID, STATUS FROM EMPLOYEE WHERE email = ? AND password = ?");
	...
	/*
	 * Implementing checkEmploye method Checking status of employee using id and
	 * password.
	 */
	...
	public boolean checkEmployee(int empID, String password) throws EmployeeException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			PreparedStatement ps = conn
					.prepareStatement("SELECT STATUS FROM EMPLOYEE WHERE empID = ? AND password = ?");
	...
	/*
	 * Implementing getComplaintID method Getting Complaint ID using foreign key
	 * empID in problem_log table
	 */
	...
	/**
	 * @param complaintID, empID
	 *@return
	 *@throws Problem Exception
	 */
	@Override
	public String showComplaintStatus(int complaintID, int empID) throws ProblemException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			PreparedStatement ps = conn
					.prepareStatement("SELECT status FROM PROBLEM_LOG WHERE logID = ? AND empID = ?");
	...
	/**
	 *@param empID,newPassword, oldPassword
	 *@return
	 *@throws EmployeeException
	 */
	@Override
	public String changePassword(int empID, String newPassword, String oldPassword) throws EmployeeException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			PreparedStatement ps = conn
					.prepareStatement("UPDATE employee SET password = ? WHERE empID = ? AND password = ?");
	...
	/**
	 *@param empID, password, probDesc, category
	 *@return
	 *@throws ProblemException
	 */
	@Override
	public String registerANewComplain(int empID, String password, String probDesc, String category)
			throws ProblemException {
		String message = "Complaint Not Registered...\nWrong Password Entered or Other";
		...
		try {
			if (checkEmployee(empID, password)) {
				try (Connection conn = DataBaseUtility.provideConnection()) {
					PreparedStatement ps = conn
							.prepareStatement("INSERT INTO PROBLEM_TEMP (probDesc, category, empID) VALUES (?, ?, ?)");
	...
	public List<ProblemEmpHistDto> getComplaintHistory(int empID) throws ProblemException {
		...
		try (Connection conn = DataBaseUtility.provideConnection()) {
			...
			while (rs.next()) {
				...
				PreparedStatement ps1 = conn
						.prepareStatement("SELECT probDesc, category FROM problem WHERE probID = ?");
		...
		return dtos;    //return list of ProblemEmpHistDto  
	}
